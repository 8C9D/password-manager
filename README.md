...
- **Encryption:** your master password is stretched with Argon2id (m = 64 MiB, t = 3, p = 1) into a 256-bit key; each entry's password and notes are encrypted with AES-256-GCM using a fresh random nonce. The master password and derived key live in memory only while the vault is unlocked and are zeroized on lock.
